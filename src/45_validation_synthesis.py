...
# Phase 3: spatial metrics (module name starts with digit -> importlib)
_SPATIAL_MOD = None
try:
    _sp_path = Path(__file__).parent / "53_spatial_metrics.py"
    _sp_spec = importlib.util.spec_from_file_location("spatial_metrics", _sp_path)
    _SPATIAL_MOD = importlib.util.module_from_spec(_sp_spec)
    _sp_spec.loader.exec_module(_SPATIAL_MOD)
except Exception:
    _SPATIAL_MOD = None
...
# =========================================================================== #
# STEP A0 - Phase 3: Spatial validation metrics (DSC + Hausdorff)
# =========================================================================== #
def _recompute_spherical_masks() -> Dict[str, np.ndarray]:
    """Recompute the deterministic isotropic-baseline final `u` masks per patient.

    Mirrors `run_spherical_baseline` but returns the final 2D density arrays
    rather than just scalar geometry metrics. Deterministic given the cached
    rho per patient (loaded from the anisotropic evolution npz files).
    """
    # Per-patient rho from anisotropic cohort (deterministic input)
    rho_pids: Dict[str, float] = {}
    for pid in COHORT_PATIENTS:
        npz_path = Path(EVOLUTION_ANISO_FMT.format(pid=pid))
        if not npz_path.exists():
            continue
        d = np.load(npz_path, allow_pickle=True)
        rho_pids[pid] = float(d["rho_field"].mean())

    masks: Dict[str, np.ndarray] = {}
    for pid in COHORT_PATIENTS:
        if pid not in rho_pids:
            continue
        rho = rho_pids[pid]
        u = _initial_seed(center=(40, 40), sigma=3.0, amplitude=0.8)
        for _ in range(N_STEPS):
...
def compute_spatial_metrics_cohort(
    force: bool = False,
) -> Dict[str, Any]:
    """Compute DSC/HD/MSD between anisotropic (predicted) and isotropic (reference)
    tumor masks for each cohort patient.

    Anisotropic masks are read from `output/anisotropic_evolution_PAT_XXXX.npz`
    (key `final_density`). Isotropic reference masks are recomputed
    deterministic via the same FK baseline used in `run_spherical_baseline`.

    Results are cached to SPATIAL_METRICS_CACHE (re-run with --force).
    """
    if _SPATIAL_MOD is None:
        print("[Spatial] Module not available; skipping spatial metrics")
        return {}

    if SPATIAL_METRICS_CACHE.exists() and not force:
        with open(SPATIAL_METRICS_CACHE) as f:
            cached = json.load(f)
        if cached.get("schema") == "month10-spatial-v1":
            print(f"[Spatial] Using cached {SPATIAL_METRICS_CACHE} "
                  f"(pass --force to rerun)")
            return cached["cohort"]

    print(f"[Spatial] Computing DSC/HD/MSD for {len(COHORT_PATIENTS)} patients...")
    iso_masks = _recompute_spherical_masks()
    ...
    for pid in COHORT_PATIENTS:
        ...
        if not aniso_npz.exists() or pid not in iso_masks:
            print(f"[Spatial] {pid}: skipping (missing evolution mask)")
            continue
        aniso_u = np.load(aniso_npz, allow_pickle=True)["final_density"]
        iso_u = iso_masks[pid]

        # Align shapes if needed (both should be 100x100)
        if aniso_u.shape != iso_u.shape:
            min_h = min(aniso_u.shape[0], iso_u.shape[0])
            min_w = min(aniso_u.shape[1], iso_u.shape[1])
            aniso_u = aniso_u[:min_h, :min_w]
            iso_u = iso_u[:min_h, :min_w]

        metrics = _SPATIAL_MOD.compute_spatial_metrics(
            aniso_u, iso_u, threshold=threshold, compute_hd95=True
        )
        per_patient[pid] = metrics
        dsc_list.append(metrics["dice_coefficient"])
        hd_list.append(metrics["hausdorff_distance_mm"])
        msd_list.append(metrics["mean_surface_distance_mm"])
        hd95_list.append(metrics["hausdorff_distance_95_mm"])
        print(f"[Spatial] {pid}: DSC={metrics['dice_coefficient']:.3f}, "
              f"HD={metrics['hausdorff_distance_mm']:.2f}mm, "
              f"HD95={metrics['hausdorff_distance_95_mm']:.2f}mm, "
              f"MSD={metrics['mean_surface_distance_mm']:.2f}mm")
    ...
    cohort_summary: Dict[str, Any] = {
        ...
        "n_patients": n,
        "dice_coefficient": {
            "mean": float(np.mean(dsc_list)) if n else 0.0,
            "std": float(np.std(dsc_list, ddof=1)) if n > 1 else 0.0,
            "min": float(np.min(dsc_list)) if n else 0.0,
            "max": float(np.max(dsc_list)) if n else 0.0,
            "values": dsc_list,
        },
        "hausdorff_distance_mm": {
            "mean": float(np.mean(hd_list)) if n else 0.0,
            "std": float(np.std(hd_list, ddof=1)) if n > 1 else 0.0,
            "min": float(np.min(hd_list)) if n else 0.0,
            "max": float(np.max(hd_list)) if n else 0.0,
            "values": hd_list,
        },
        "hausdorff_distance_95_mm": {
            "mean": float(np.mean(hd95_list)) if n else 0.0,
            "std": float(np.std(hd95_list, ddof=1)) if n > 1 else 0.0,
            "values": hd95_list,
        },
        "mean_surface_distance_mm": {
            "mean": float(np.mean(msd_list)) if n else 0.0,
            "std": float(np.std(msd_list, ddof=1)) if n > 1 else 0.0,
            "values": msd_list,
        },
        "meets_clinical_threshold": bool(
            bool(n) and np.mean(dsc_list) >= 0.7
        ),
    }

    cache = {"schema": "month10-spatial-v1", "cohort": cohort_summary}
    with open(SPATIAL_METRICS_CACHE, "w") as f:
        json.dump(_json_clean(cache), f, indent=2)
    print(f"[Spatial] DSC mean={cohort_summary['dice_coefficient']['mean']:.3f} "
          f"+/- {cohort_summary['dice_coefficient']['std']:.3f}; "
          f"HD mean={cohort_summary['hausdorff_distance_mm']['mean']:.2f}mm")
    print(f"[Spatial] Cached -> {SPATIAL_METRICS_CACHE}")
...
def render_master_canvas(master: Dict[str, Any],
    ...
    Extended to 3x2 with Phase 3 spatial metrics row (E, F, G).
    """
    path.parent.mkdir(parents=True, exist_ok=True)
    patients = master["patients"]
    ...
    # 3 rows: A/B top, C/D middle, E/F/G bottom (spatial metrics)
    fig = plt.figure(figsize=(16, 20))
    gs = fig.add_gridspec(3, 2, hspace=0.34, wspace=0.24,
    ...
    # ------------------------------------------------------------------ #
    # Phase 3: Panel E, F, G - Spatial validation metrics (new row)
    # ------------------------------------------------------------------ #
    spatial = master.get("spatial_metrics", {})
    if spatial and spatial.get("n_patients", 0) > 0:
        ss_e = gs[2, 0]
        ss_f = gs[2, 1]
        sub_e = ss_e.subgridspec(1, 2, wspace=0.3)
        sub_f = ss_f.subgridspec(1, 2, wspace=0.3)

        per_pat = spatial.get("per_patient", {})
        pids = list(per_pat.keys())
        dsc_vals = [per_pat[p]["dice_coefficient"] for p in pids]
        hd_vals = [per_pat[p]["hausdorff_distance_mm"] for p in pids]
        hd95_vals = [per_pat[p]["hausdorff_distance_95_mm"] for p in pids]
        msd_vals = [per_pat[p]["mean_surface_distance_mm"] for p in pids]

        # Panel E (left): DSC cohort bar chart with mean ± std
        ax_e1 = fig.add_subplot(sub_e[0, 0])
        colors_e = ["#2E86AB" if v >= 0.7 else "#A23B72" for v in dsc_vals]
        x = np.arange(len(pids))
        ax_e1.bar(x, dsc_vals, color=colors_e, edgecolor="white", linewidth=0.5)
        ax_e1.axhline(spatial["dice_coefficient"]["mean"], color="k", ls="--", lw=1,
                      label=f"Mean = {spatial['dice_coefficient']['mean']:.3f}")
        ax_e1.fill_between([-0.5, len(pids)-0.5],
                           spatial["dice_coefficient"]["mean"] - spatial["dice_coefficient"]["std"],
                           spatial["dice_coefficient"]["mean"] + spatial["dice_coefficient"]["std"],
                           color="gray", alpha=0.15, label=f"±1 SD")
        ax_e1.axhline(0.7, color="red", ls=":", lw=1.5, label="Clinical threshold (0.70)")
        ax_e1.set_xticks(x)
        ax_e1.set_xticklabels(pids, rotation=45, fontsize=7)
        ax_e1.set_ylabel("Dice Similarity Coefficient")
        ax_e1.set_ylim(0, 1.05)
        ax_e1.set_title("E. DSC: Anisotropic vs Isotropic", fontsize=10, fontweight="bold", loc="left")
        ax_e1.grid(alpha=0.3, axis="y")
        ax_e1.legend(fontsize=6, loc="lower right")

        # Panel E (right): HD cohort bar chart with 5mm threshold
        ax_e2 = fig.add_subplot(sub_e[0, 1])
        colors_f = ["#27AE60" if v <= 5.0 else "#E74C3C" for v in hd_vals]
        ax_e2.bar(x, hd_vals, color=colors_f, edgecolor="white", linewidth=0.5,
                  label="HD (max)")
        ax_e2.bar(x, hd95_vals, color=colors_f, edgecolor="white", linewidth=0.5,
                  alpha=0.6, label="HD95")
        ax_e2.axhline(spatial["hausdorff_distance_mm"]["mean"], color="k", ls="--", lw=1,
                      label=f"Mean HD = {spatial['hausdorff_distance_mm']['mean']:.1f}mm")
        ax_e2.axhline(5.0, color="red", ls=":", lw=1.5, label="Clinical threshold (5mm)")
        ax_e2.set_xticks(x)
        ax_e2.set_xticklabels(pids, rotation=45, fontsize=7)
        ax_e2.set_ylabel("Hausdorff Distance (mm)")
        ax_e2.set_title("F. HD: Anisotropic vs Isotropic", fontsize=10, fontweight="bold", loc="left")
        ax_e2.grid(alpha=0.3, axis="y")
        ax_e2.legend(fontsize=6, loc="upper right")

        # Panel G: Scatter DSC vs HD (on the second subgrid's left spot)
        # Actually use the remaining space in the second subgrid
        ax_g = fig.add_subplot(sub_f[0, 0])
        ax_g.scatter(hd_vals, dsc_vals, s=80, c="#34495E", edgecolors="white", linewidth=1, alpha=0.8)
        # Add correlation line
        if len(dsc_vals) > 2:
            from scipy import stats as sp_stats
            r, p = sp_stats.pearsonr(hd_vals, dsc_vals)
            ax_g.text(0.05, 0.95, f"r = {r:.3f}\np = {p:.3g}",
                      transform=ax_g.transAxes, fontsize=8,
                      va="top", ha="left",
                      bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="gray", alpha=0.8))
        ax_g.axvline(5.0, color="red", ls=":", lw=1.5, label="HD threshold 5mm")
        ax_g.axhline(0.7, color="red", ls=":", lw=1.5, label="DSC threshold 0.7")
        ax_g.set_xlabel("Hausdorff Distance (mm)")
        ax_g.set_ylabel("Dice Similarity Coefficient")
        ax_g.set_title("G. DSC vs HD Correlation", fontsize=10, fontweight="bold", loc="left")
        ax_g.grid(alpha=0.3)
        ax_g.legend(fontsize=6, loc="lower left")

        # G-right: Mean surface distance (use the second column)
        ax_g2 = fig.add_subplot(sub_f[0, 1])
        ax_g2.bar(x, msd_vals, color="#8E44AD", edgecolor="white", linewidth=0.5)
        ax_g2.axhline(spatial["mean_surface_distance_mm"]["mean"], color="k", ls="--", lw=1,
                      label=f"Mean MSD = {spatial['mean_surface_distance_mm']['mean']:.2f}mm")
        ax_g2.set_xticks(x)
        ax_g2.set_xticklabels(pids, rotation=45, fontsize=7)
        ax_g2.set_ylabel("Mean Surface Distance (mm)")
        ax_g2.set_title("H. Mean Surface Distance", fontsize=10, fontweight="bold", loc="left")
        ax_g2.grid(alpha=0.3, axis="y")
        ax_g2.legend(fontsize=6, loc="upper right")
...
def write_poster_findings(master: Dict[str, Any],
    ...
    # Phase 4: Dynamically generated sections for upgraded capabilities
    spatial = master.get("spatial_metrics", {})
    if spatial and spatial.get("n_patients", 0) > 0:
        bullets.append(
        ...
        for p in master["patients"]:
            pid = p["patient_id"]
            # These would be populated if --estimate-params was used
            # Placeholder template for patient-specific values
            bullets.append(
                f"- Patient {pid}: ρ = {{rho:.3f}} /day (95% CI: {{ci_low:.3f}}-{{ci_high:.3f}})"
            )
            bullets.append(
                f"- Patient {pid}: D = {{D:.4f}} mm²/day (95% CI: {{ci_low:.4f}}-{{ci_high:.4f}})"
            )

    robust = master.get("robust_mpc", {})
    if robust:
        bullets.append(
            f"\n## Robust MPC Performance (Tier 2)"
        )
        bullets.append(
            f"- Cost variance reduction: {{variance_reduction:.1f}}% vs standard MPC"
        )
        bullets.append(
            f"- Adaptive horizon range: {{min_horizon}}-{{max_horizon}} days"
        )

    if spatial and spatial.get("n_patients", 0) > 0:
        dsc_mean = spatial["dice_coefficient"]["mean"]
        dsc_std = spatial["dice_coefficient"]["std"]
        hd_mean = spatial["hausdorff_distance_mm"]["mean"]
        hd_std = spatial["hausdorff_distance_mm"]["std"]
        bullets.append(
            f"\n## Spatial Validation (Tier 3)"
        )
        bullets.append(
            f"- Mean DSC: {dsc_mean:.2f} ± {dsc_std:.2f}"
        )
        bullets.append(
            f"- Mean HD: {hd_mean:.1f} ± {hd_std:.1f} mm"
        )
        bullets.append(
            f"- Meets clinical threshold (DSC≥0.7, HD≤5mm): {spatial.get('meets_clinical_threshold', False)}"
        )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # 3b. Phase 3: Spatial validation metrics (DSC, HD, MSD)
    spatial = compute_spatial_metrics_cohort(force=force)
    if spatial:
        master["spatial_metrics"] = spatial
