#!/usr/bin/env python3
"""
Lightweight 4D 3D Tumor Viewer - Small, Fast, Works
====================================================
Only embeds key frames (Day 0, 30, 60, 90, 120) with reduced resolution.
"""
...
def main():
    ...
    print(f"Building lightweight 3D viewer for {target_patient}...")
    ...
    u = u[::4, ::4, ::4]  # 4x downsample for smaller data
    ...
    dt = 1.0
    total_days = 120
    save_interval = 30  # Only 5 key frames: 0, 30, 60, 90, 120
    ...
    # Run simulation and collect only key frames
    os.makedirs("output/time_series_3d_light", exist_ok=True)
    for f in Path("output/time_series_3d_light").glob("*.npy"):
        f.unlink()
    
    np.save("output/time_series_3d_light/tumor_3d_day_000.npy", u)
    ...
    print(f"  [SIM] Collected {len(snapshots)} key frames: {day_labels}")
    
    # Encode snapshots as base64
    ...
    # Value range - handle binary data case
    all_vals = np.concatenate([s.flatten() for s in snapshots])
    ...
    if len(all_nonzero) > 0:
        ...
        # Handle binary case where all values are 1.0
        if vmin == vmax:
            vmin = 0.5
            vmax = 1.0
    else:
    ...
    iso_levels = np.linspace(vmin, vmax, 3).tolist()  # Only 3 levels
...
<body>
    ...
    <script>
        ...
        function createIsosurfaceTraces(snapshotData) {{
            ...
            for (let i = 0; i < isoLevels.length; i++) {{
                traces.push({{
                    ...
                    showscale: i === 0,
                    colorbar: i === 0 ? {{title: 'Density', thickness: 20, len: 0.75}} : undefined
                }});
            }}
            return traces;
        }}
        ...
        function updatePlot(frameIdx) {{
            ...
            Plotly.react('plot', traces, {{
                ...
                title: {{text: '4D Tumor - {target_patient} (Day ' + dayLabels[frameIdx] + ')', font: {{size: 16}}}},
    ...
    output_path = BASE_OUTPUT / f"4d_3d_light_{target_patient}.html"
    with open(output_path, 'w') as f:
        f.write(html)
    
    print(f"[SUCCESS] Lightweight viewer saved: {output_path}")
