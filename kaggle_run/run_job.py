...
T_THERAPY_DAYS = 60.0 if not LOCAL_TEST else 10.0  # therapy horizon (days)
CFL_SAFETY = 0.25           # explicit Euler safety factor
...
def step_anisotropic_3d(
    ...
    dx: float, dt: float, K_drug: Optional[np.ndarray] = None,
    K: float = K_CARRYING,
) -> np.ndarray:
    diff = anisotropic_divergence(c, tensor, dx)
    react = rho * c * (1.0 - c / K)
    if K_drug is not None:
        c_new = c + dt * (diff + react - K_drug * c)
    else:
...
def run_phase3(out_dir: Path) -> Dict:
    ...
    for step in range(n_steps):
        c = step_anisotropic_3d(c, tensor, rho, DX_MM, dt, K_drug=None)
...
# =============================================================================
# PHASE 4: VIRTUAL THERAPY SOLVER (inline)
# =============================================================================
...
def _load_phase3_tensor(out_dir: Path) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if tensor_path.exists():
        ...
        return tensor, FA, e1
    print("[PHASE 4] Phase 3 tensor not found; reconstructing from Phase 3 logic...")
    return _reconstruct_tensor_from_phase3()
...
def _reconstruct_tensor_from_phase3() -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    nx, ny, nz = GRID_3D
    X, Y, Z = np.meshgrid(
        np.linspace(-1, 1, nx), np.linspace(-1, 1, ny), np.linspace(-1, 1, nz),
        indexing="ij",
    ...
    D0_eff = np.full(GRID_3D, 0.05, dtype=np.float64)
    d0 = (1.0 - FA) * D0_eff
    d_d = 3.0 * FA * D0_eff
...
def _drug_kill_rate(t: float, c: np.ndarray, c_max: float,
                    tensor: np.ndarray, e1: np.ndarray) -> np.ndarray:
...
def compute_therapy_metrics(
    c_treated: np.ndarray, c_untreated: np.ndarray, tensor: np.ndarray,
    e1: np.ndarray, dx: float,
) -> Dict:
    nx, ny, nz = c_treated.shape
    center = np.array([nx//2, ny//2, nz//2])
    u_max = float(np.max(c_treated))
    threshold = max(0.20 * u_max, 1e-3)
    mask = c_treated >= threshold
    ...
    if not np.any(mask):
        return {
            ...
            "log_kill": 0.0,
            "time_to_recurrence_days": float(T_THERAPY_DAYS),
            "spatial_selectivity_index": 0.0,
            "max_invasion_x_mm": 0.0, "max_invasion_y_mm": 0.0, "max_invasion_z_mm": 0.0,
            "peak_density_treated": u_max, "peak_density_untreated": float(np.max(c_untreated)),
        }
    ...
    vol_treated = float(np.sum(c_treated > threshold) * dx ** 3)
    vol_untreated = float(np.sum(c_untreated > threshold) * dx ** 3)
    vol_reduction = 100.0 * (vol_untreated - vol_treated) / (vol_untreated + 1e-12)

    log_kill = np.log10(vol_untreated / (vol_treated + 1e-12)) if vol_treated > 0 else 99.0

    # Time to recurrence: how many days post-therapy to regrow to 50% of untreated volume
    # Approximate: if treated volume is V_t and regrowth rate is ~0.1/day, time ~ log(0.5*V_u/V_t)/rate
    regrowth_rate = 0.1  # day^-1, approximate
    if vol_treated > 0 and vol_untreated > vol_treated:
        time_to_recurrence = np.log(0.5 * vol_untreated / vol_treated) / regrowth_rate
    else:
        time_to_recurrence = T_THERAPY_DAYS

    boundary = c_treated >= threshold
    boundary[0,:,:] = boundary[-1,:,:] = False
    boundary[:,0,:] = boundary[:,-1,:] = False
    boundary[:,:,0] = boundary[:,:,-1] = False
    pref_angle = 0.0
    if np.any(boundary):
    ...
    return {
        ...
        "time_to_recurrence_days": float(time_to_recurrence),
        "spatial_selectivity_index": 0.0,
        "max_invasion_x_mm": dist_x,
        "max_invasion_y_mm": dist_y,
        "max_invasion_z_mm": dist_z,
        ...
        "preferred_migration_angle_deg": pref_angle,
        "peak_density_treated": u_max,
        "peak_density_untreated": float(np.max(c_untreated)),
        "therapy_horizon_days": T_THERAPY_DAYS,
        "grid": list(GRID_3D),
        "dx_mm": dx,
    }
...
def save_therapy_visualization(
    ...
    threshold = 0.2 * float(np.max(c_treated))
    ...
    fig.suptitle(f"Phase 4: Virtual Therapy (T={T_THERAPY_DAYS} days)",
                 fontsize=13, fontweight="bold", y=1.01)
    fig.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"[SAVE] Figure -> {out_path}")
...
def run_phase4(out_dir: Path, rho: np.ndarray) -> Dict:
    out_dir.mkdir(parents=True, exist_ok=True)

    print("[PHASE 4] Loading Phase 3 anisotropic concentration and tensor field...")
    ...
    c_untreated = c_init.copy()
    for step in range(n_steps):
        t = step * T_THERAPY_DAYS / n_steps
        ux, uy, uz = grad_3d(c_untreated, DX_MM)
        Jx, Jy, Jz = -tensor[..., 0, 0] * ux, -tensor[..., 1, 1] * uy, -tensor[..., 2, 2] * uz
        diff = -div_3d(Jx, Jy, Jz, DX_MM)
        react = rho * c_untreated * (1.0 - c_untreated / K_CARRYING)
        c_untreated = c_untreated + T_THERAPY_DAYS / n_steps * (diff + react)
        c_untreated = np.clip(c_untreated, 0.0, K_CARRYING)
        c_untreated[0,:,:] = c_untreated[1,:,:]; c_untreated[-1,:,:] = c_untreated[-2,:,:]
        c_untreated[:,0,:] = c_untreated[:,1,:]; c_untreated[:,-1,:] = c_untreated[:,-2,:]
        c_untreated[:,:,0] = c_untreated[:,:,1]; c_untreated[:,:,-1] = c_untreated[:,:,-2]

    # Treated simulation
    print(f"[PHASE 4] Running 3D treated simulation: {n_steps} steps, dt={T_THERAPY_DAYS/n_steps:.5f} days...")
    c_treated = c_init.copy()
    report_every = max(1, n_steps // 10)
    for step in range(n_steps):
        t = step * T_THERAPY_DAYS / n_steps
        K_drug = _drug_kill_rate(t, c_treated, c_treated.max(), tensor, e1)
        c_treated = step_anisotropic_3d(c_treated, tensor, rho, DX_MM, T_THERAPY_DAYS/n_steps, K_drug)
        if step % report_every == 0 or step == n_steps - 1:
            vol = float(np.sum(c_treated) * DX_MM ** 3)
            print(f"  step {step}/{n_steps} (t={t:.1f}d): volume={vol:.2f} mm^3, max_c={c_treated.max():.3f}")

    print("[PHASE 4] Computing therapy metrics...")
    metrics = compute_therapy_metrics(c_treated, c_untreated, tensor, e1, DX_MM)
    print(f"  Volume reduction: {metrics['volume_reduction_pct']:.1f}%")
    print(f"  Log-kill: {metrics['log_kill']:.2f}")
    print(f"  Time-to-recurrence: {metrics['time_to_recurrence_days']:.1f} days")

    np.save(out_dir / "phase4_treated_concentration.npy", c_treated)
    np.save(out_dir / "phase4_untreated_concentration.npy", c_untreated)
    save_therapy_visualization(c_treated, c_untreated, tensor, FA, e1, metrics,
...
def main():
    ...
    # --- Phase 4 ---
    p4_start = time.time()
    print("[PHASE 4] Loading Phase 3 outputs for virtual therapy simulation...")
    p4_metrics = run_phase4(out_dir, rho)
    
    print(f"[PHASE 4] Done in {time.time() - p4_start:.1f}s")
    ...
    print(f"  Therapy volume reduction: {p4_metrics.get('volume_reduction_pct', 0):.1f}%")
    print(f"  Therapy log-kill: {p4_metrics.get('log_kill', 0):.2f}")
