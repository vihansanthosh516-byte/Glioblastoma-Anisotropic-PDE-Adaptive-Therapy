...
All numeric values are sourced from `output/master_cohort_summary.json` and computed at write time. Honest framing per decisions D2/D4 (adaptive maintains comparable TTP at lower drug exposure; not superior in TTP).
...
- Within the virtual cohort, adaptive therapy maintained comparable simulated time-to-progression (TTP) while significantly reducing cumulative drug exposure (paired t = 1.57, p=0.1594; TTP ratio mean = 0.763, range 0.256-1.074) at 9-21% lower cumulative drug exposure (mean ± SD = 14.9 ± 4.1%).
